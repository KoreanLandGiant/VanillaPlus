class VanillaPlusView_Frontend_Customization extends AOCView_Frontend_Customization
config(VanillaPlusCustomization);
...
function PopulateCustomizationInfoArrays()
{
	...
	//temp = class'VanillaPlusCustomization'.static.CheckMicroTxOwnership(1, repInfo);
	//class'WorldInfo'.static.GetWorldInfo().Spawn.
	for(i = 0; i < 4; ++i)
	{
		TeamCustomizationChoices[EFAC_Agatha].ClassCustomizationChoices[i] = class'VanillaPlusCustomization'.static.LocalGetCustomizationChoices(EFAC_Agatha, i);
		TeamCustomizationChoices[EFAC_Mason].ClassCustomizationChoices[i] = class'VanillaPlusCustomization'.static.LocalGetCustomizationChoices(EFAC_Mason, i);
		TeamCustomizationChoices[EFAC_FFA].ClassCustomizationChoices[i] = class'VanillaPlusCustomization'.static.LocalGetCustomizationChoices(EFAC_FFA, i);
	}

	for(i = 0; i < 4; ++i)
	{
		class'VanillaPlusCustomization'.static.LocalGetSelectedWeaponDrops(EFAC_MASON, i, TeamWeaponChoices[EFAC_MASON].Classes[i].Weapons, AOCPRI(repInfo));
		class'VanillaPlusCustomization'.static.LocalGetSelectedWeaponDrops(EFAC_AGATHA, i, TeamWeaponChoices[EFAC_AGATHA].Classes[i].Weapons, AOCPRI(repInfo));
		class'VanillaPlusCustomization'.static.LocalGetSelectedWeaponDrops(EFAC_FFA, i, TeamWeaponChoices[EFAC_FFA].Classes[i].Weapons, AOCPRI(repInfo));
	}

	FactionSupportFavIconId = class'VanillaPlusCustomization'.static.LocalGetFactionSupporterId();
}
...
function SaveChoices()
{
	...
	for(i = 0; i < 4; ++i)
	{
		class'VanillaPlusCustomization'.static.LocalSetCustomizationChoices(TeamCustomizationChoices[EFAC_Agatha].ClassCustomizationChoices[i], EFAC_Agatha, i, TeamWeaponChoices[EFAC_AGATHA].Classes[i].Weapons, FactionSupportFavIconId);
		class'VanillaPlusCustomization'.static.LocalSetCustomizationChoices(TeamCustomizationChoices[EFAC_Mason].ClassCustomizationChoices[i], EFAC_Mason, i, TeamWeaponChoices[EFAC_Mason].Classes[i].Weapons, FactionSupportFavIconId);
		class'VanillaPlusCustomization'.static.LocalSetCustomizationChoices(TeamCustomizationChoices[EFAC_FFA].ClassCustomizationChoices[i], EFAC_FFA, i, TeamWeaponChoices[EFAC_FFA].Classes[i].Weapons, FactionSupportFavIconId);
	}
...
function OnFadeOutDone()
{
	...
	PreviewPawn = class'WorldInfo'.static.GetWorldInfo().Spawn(class'VanillaPlusPreviewPawn',,,PawnLocation);
...
function GFxObject PopulatePatternList(int CharID, out float selectedIndex)
{
	...
	if(RightTabMode == RTAB_Pattern)
	{
		...
		IDs = class'VanillaPlusCustomization'.static.GetAllTabardsFor(CharID);
		i = 0;
		foreach IDs(GearID)
		{
			MicroTxID = 0;
			...
			bIsOwned = class'VanillaPlusCustomization'.static.IsTabardOwnedBy(GearID, CharID, class'WorldInfo'.static.GetWorldInfo().GetALocalPlayerController().PlayerReplicationInfo, SelectedClass);
			if(bIsOwned || class'VanillaPlusCustomization'.static.IsTabardVisibleIfUnowned(GearID, CharID))
			{
				IndvElement = CreateObject("Object");
				IndvElement.SetInt("tabardID", GearID);
				IndvElement.SetBool("isLocked", !bIsOwned);
				class'VanillaPlusCustomization'.static.GetTabardStoreDescription(GearID, CharID, bPromptToBuy, Price, MicroTxID, PurchaseName);
				...
				DiscountPercent = "";
				if(class'VanillaPlusCustomization'.static.IsMicroTxnItemOnSale(MicroTxID, Price, DiscountPercent))
				{
				...
				else
				{
					...
					if(class'VanillaPlusCustomization'.static.IsMicroTxnItemNew(MicroTxID))
					{
				...
				IndvElement.SetString("itemName", DiscountPercent@class'VanillaPlusCustomization'.static.GetTabardName(GearID, CharID));
				...
				if(GearID == TeamCustomizationChoices[SelectedTeam].ClassCustomizationChoices[SelectedClass].Tabard)
				{
					...
					selectedIndex = i;
					SetItemNameLabel(class'VanillaPlusCustomization'.static.GetTabardName(GearID, CharID));
				}
...
function GFxObject PopulateShieldList(int CharID, out float selectedIndex)
{
	...
	if(RightTabMode == RTAB_Shield)
	{
		...
		{
			class'VanillaPlusCustomization'.static.GetShieldDisplayInfo(TeamWeaponChoices[SelectedTeam].Classes[SelectedClass].Weapons[SelectedWeapon], class<AOCWeapon_Shield>(WeaponClasses[SelectedWeapon]).default.Shield, AllowedTeams, SkeletalMeshPath, StaticMeshPath, MaterialPath, bUseDefaultParameters, WeaponParameterSets, ShieldPatternNames, ShieldPatternPaths);
		...
		{
			IDs = class'VanillaPlusCustomization'.static.GetAllShieldPatternsFor(CharID);
			i = 0;
			foreach IDs(GearID)
			{
				MicroTxID = 0;
				...
				bIsOwned = class'VanillaPlusCustomization'.static.IsShieldPatternOwnedBy(GearID, CharID, class'WorldInfo'.static.GetWorldInfo().GetALocalPlayerController().PlayerReplicationInfo, SelectedClass);
				if(bIsOwned || class'VanillaPlusCustomization'.static.IsShieldPatternVisibleIfUnowned(GearID, CharID))
				{
					IndvElement = CreateObject("Object");
					IndvElement.SetInt("shieldID", GearID);
					IndvElement.SetBool("isLocked", !bIsOwned);
					class'VanillaPlusCustomization'.static.GetShieldStoreDescription(GearID, CharID, bPromptToBuy, Price, MicroTxID, PurchaseName);
					...
					IndvElement.SetBool("isPurchasable", Bool(bPromptToBuy) && !bIsOwned);
					IndvElement.SetString("price", Price);
					IndvElement.SetString("itemName", DiscountPercent@class'VanillaPlusCustomization'.static.GetShieldPatternName(GearID, CharID));
					IndvElement.SetInt("microTxnId", MicroTxID);
					ListDataProvider.SetElementObject(i, IndvElement);
					...
					if(GearID == TeamCustomizationChoices[SelectedTeam].ClassCustomizationChoices[SelectedClass].Shield)
					{
						ShowBuyButton(Bool(bPromptToBuy) && !bIsOwned, Price);
						selectedIndex = i;
						SetItemNameLabel(class'VanillaPlusCustomization'.static.GetShieldPatternName(GearID, CharID));
					}
...
function GFxObject PopulateEmblemList(out float selectedIndex)
{
	...
	if(RightTabMode == RTAB_Emblem)
	{
		...
		IDs = class'VanillaPlusCustomization'.static.GetAllEmblemsFor(SelectedTeam);
		i = 0;
		foreach IDs(GearID)
		{
			MicroTxID = 0;
			...
			PurchaseName = "";
			Price = "";
			bIsMicoTxnVisible = class'VanillaPlusCustomization'.static.IsMicroTxnEmblemVisible(GearID, SelectedTeam, class'WorldInfo'.static.GetWorldInfo().GetALocalPlayerController().PlayerReplicationInfo);

			if(bIsMicoTxnVisible)
			{
				bIsOwned = class'VanillaPlusCustomization'.static.IsEmblemOwnedBy(GearID, SelectedTeam, class'WorldInfo'.static.GetWorldInfo().GetALocalPlayerController().PlayerReplicationInfo, SelectedClass);
				if(bIsOwned || class'VanillaPlusCustomization'.static.IsEmblemVisibleIfUnowned(GearID, SelectedTeam))
				{
					IndvElement = CreateObject("Object");
					IndvElement.SetInt("emblemID", GearID);
					IndvElement.SetBool("isLocked", !bIsOwned);
					class'VanillaPlusCustomization'.static.GetEmblemStoreDescription(GearID, SelectedTeam, bPromptToBuy, Price, MicroTxID, PurchaseName);
					...
					IndvElement.SetBool("isPurchasable", Bool(bPromptToBuy) && !bIsOwned);
					IndvElement.SetInt("microTxnId", MicroTxID);
					IndvElement.SetString("itemName", DiscountPercent@class'VanillaPlusCustomization'.static.GetEmblemName(GearID, SelectedTeam));
					ListDataProvider.SetElementObject(i, IndvElement);
					...
					if(GearID == TeamCustomizationChoices[SelectedTeam].ClassCustomizationChoices[SelectedClass].Emblem)
					{
						ShowBuyButton(Bool(bPromptToBuy) && !bIsOwned, Price);
						selectedIndex = i;
						SetItemNameLabel(class'VanillaPlusCustomization'.static.GetEmblemName(GearID, SelectedTeam));
					}
	...
	if(RightTabMode == RTAB_Weapon)
	{
		...
		foreach ClassWeapons(WeaponClass)
		{
			...
			if(AllOwnedOrPurchasableWeaponDrops[WeaponClass.default.CurrentWeaponType].Items.Length > 0)
			{
				...
				//class'VanillaPlusCustomization'.static.GetWeaponSkinStoreDescription(GearID, CharacterID, bPromptToBuy, Price, MicroTxID, PurchaseName);
				...
				{
					if(class'VanillaPlusCustomization'.static.IsMicroTxnItemOnSale(WeaponItem.GearData.MicroTxID, Price, DiscountPercent))
					{
						IsOnSale = true;
					}
					else if(class'VanillaPlusCustomization'.static.IsMicroTxnItemNew(WeaponItem.GearData.MicroTxID))
					{
						IsNew = true;
					}
...
function GFxObject PopulateHelmetList(int CharID, out float selectedIndex)
{
	...
	if(RightTabMode == RTAB_Helmet)
	{
		...
		IDs = class'VanillaPlusCustomization'.static.GetAllHelmetsFor(CharID);
		i = 0;
		foreach IDs(GearID)
		{
			MicroTxID = 0;
			...
			bIsOwned = class'VanillaPlusCustomization'.static.IsHelmetOwnedBy(GearID, CharID, class'WorldInfo'.static.GetWorldInfo().GetALocalPlayerController().PlayerReplicationInfo, SelectedClass);

			bIsMicoTxnVisible = class'VanillaPlusCustomization'.static.IsMicroTxnHelmetVisible(GearID, CharID, class'WorldInfo'.static.GetWorldInfo().GetALocalPlayerController().PlayerReplicationInfo);

			if(bIsMicoTxnVisible)
			{
				if(bIsOwned || class'VanillaPlusCustomization'.static.IsHelmetVisibleIfUnowned(GearID, CharID))
				{
					IndvElement = CreateObject("Object");
					IndvElement.SetInt("helmetID", GearID);
					IndvElement.SetBool("isLocked", !bIsOwned);
					class'VanillaPlusCustomization'.static.GetHelmetStoreDescription(GearID, CharID, bPromptToBuy, Price, MicroTxID, PurchaseName);
					...
					IndvElement.SetBool("isPurchasable", Bool(bPromptToBuy) && !bIsOwned);
					IndvElement.SetInt("microTxnId", MicroTxID);
					IndvElement.SetString("itemName", DiscountPercent@class'VanillaPlusCustomization'.static.GetHelmetName(GearID,TeamCustomizationChoices[SelectedTeam].ClassCustomizationChoices[SelectedClass].Character));
					ListDataProvider.SetElementObject(i, IndvElement);
					...
					if(GearID == TeamCustomizationChoices[SelectedTeam].ClassCustomizationChoices[SelectedClass].Helmet)
					{
						ShowBuyButton(Bool(bPromptToBuy) && !bIsOwned, Price);
						selectedIndex = i;
						SetItemNameLabel(class'VanillaPlusCustomization'.static.GetHelmetName(GearID,TeamCustomizationChoices[SelectedTeam].ClassCustomizationChoices[SelectedClass].Character));
					}
...
function GFxObject PopulateArmourList(out float selectedIndex)
{
	...
	if(RightTabMode == RTAB_Armour)
	{
		...
		IDs = class'VanillaPlusCustomization'.static.GetAllCharactersFor(SelectedTeam, SelectedClass);
		i = 0;
		foreach IDs(GearID)
		{
			MicroTxID = 0;
			...
			bIsOwned = class'VanillaPlusCustomization'.static.IsCharacterOwnedBy(GearID, SelectedTeam, SelectedClass, class'WorldInfo'.static.GetWorldInfo().GetALocalPlayerController().PlayerReplicationInfo);
			bIsMicoTxnVisible = class'VanillaPlusCustomization'.static.IsMicroTxnCharacterVisible(GearID, SelectedTeam, SelectedClass, class'WorldInfo'.static.GetWorldInfo().GetALocalPlayerController().PlayerReplicationInfo);

			if(bIsMicoTxnVisible)
			{
				if(bIsOwned || class'VanillaPlusCustomization'.static.IsCharacterVisibleIfUnowned(GearID, SelectedTeam, SelectedClass))
				{
					IndvElement = CreateObject("Object");
					IndvElement.SetInt("characterID", GearID);
					IndvElement.SetBool("isLocked", !bIsOwned);
					class'VanillaPlusCustomization'.static.GetCharacterStoreDescription(GearID, bPromptToBuy, Price, MicroTxID, PurchaseName);
					...
					IndvElement.SetBool("isPurchasable", Bool(bPromptToBuy) && !bIsOwned);
					IndvElement.SetInt("microTxnId", MicroTxID);
					IndvElement.SetString("itemName", DiscountPercent@class'VanillaPlusCustomization'.static.GetCharacterName(GearID));
					ListDataProvider.SetElementObject(i, IndvElement);
					...
					if(GearID == TeamCustomizationChoices[SelectedTeam].ClassCustomizationChoices[SelectedClass].Character)
					{
						ShowBuyButton(Bool(bPromptToBuy) && !bIsOwned, Price);
						selectedIndex = i;
						SetItemNameLabel(class'VanillaPlusCustomization'.static.GetCharacterName(GearID));
					}		
	...
	if(RightTabMode == RTAB_Weapon)
	{
		...
		foreach Weapons.Items(Weapon)
		{
			...
			if(bIsMicoTxnVisible && (SelectedTeam == EFAC_FFA || WeaponItemInfo.AllowedTeams.Find(SelectedTeam) != INDEX_NONE))
			{
				...
				{
					IndvElement = CreateObject("Object");
					class'VanillaPlusCustomization'.static.GetGearStoreDescription(Weapon.GearData, bPromptToBuy, Price, MicroTxID, PurchaseName);

					DiscountPercent = "";
					if(class'VanillaPlusCustomization'.static.IsMicroTxnItemOnSale(Weapon.GearData.MicroTxID, Price, DiscountPercent))
					{
						IndvElement.SetBool("isItemOnSale", true);
						IndvElement.SetBool("isNewItem", false);
						AddItemToNotificationArray(MicroTxID);
					...
					{
						IndvElement.SetBool("isItemOnSale", false);
						if(class'VanillaPlusCustomization'.static.IsMicroTxnItemNew(Weapon.GearData.MicroTxID))
						{
							IndvElement.SetBool("isNewItem", true);
							AddItemToNotificationArray(MicroTxID);
						}
...
function SetupColors(optional bool UpdateSwatch1 = true, optional bool UpdateSwatch2 = true, optional bool UpdateSwatch3 = true)
{
	...
	{		
		NumberOfEmblemColours = class'VanillaPlusCustomization'.static.GetNumberOfEmblemColours(SelectedTeam);
		NumberOfTabardColours = class'VanillaPlusCustomization'.static.GetNumberOfTabardColours(ColourSwatchIndex, SelectedTeam);
		...
		for(j = 0; j < (RightTabMode == RTAB_Emblem) ? NumberOfEmblemColours : NumberOfTabardColours; ++j)
		{
			...
			if(RightTabMode == RTAB_Emblem)
			{
				swatchColour = class'VanillaPlusCustomization'.static.ConvertEmblemColorIndexToColor(j, SelectedTeam);
			...
			{
				swatchColour = class'VanillaPlusCustomization'.static.ConvertTabardColorIndexToColor(j, SelectedTeam, ColourSwatchIndex);
				ColourObj.SetInt("ra", swatchColour.R);
				ColourObj.SetInt("ba", swatchColour.B);
				ColourObj.SetInt("ga", swatchColour.G);
				SwatchPopup[ColourSwatchIndex].AddColour(ColourObj);
...
function OnRightListItemClicked(GFxClikWidget.EventData Params)
{
	...
	if(RightTabMode == RTAB_Pattern)
	{
		...
		SetItemNameLabel(class'VanillaPlusCustomization'.static.GetTabardName(GearID, CharacterID));
	}
	...
	else if(RightTabMode == RTAB_Shield)
	{
		...
		SetItemNameLabel(class'VanillaPlusCustomization'.static.GetShieldPatternName(GearID, CharacterID));
	}
	...
	else if(RightTabMode == RTAB_Helmet)
	{
		...
		SetItemNameLabel(class'VanillaPlusCustomization'.static.GetHelmetName(GearID,CharacterID));
		FullPawnRefresh = true;
		FullRefreshDisplayedPawn();
	}
...
function CheckLocks()
{
	...
	if(RightTabMode == RTAB_Emblem && !class'VanillaPlusCustomization'.static.IsEmblemOwnedBy(TeamCustomizationChoices[SelectedTeam].ClassCustomizationChoices[SelectedClass].Emblem, SelectedTeam, class'WorldInfo'.static.GetWorldInfo().GetALocalPlayerController().PlayerReplicationInfo, SelectedClass))
	{
		LockText = class'VanillaPlusCustomization'.static.GetEmblemStoreDescription(TeamCustomizationChoices[SelectedTeam].ClassCustomizationChoices[SelectedClass].Emblem, SelectedTeam, bPromptToBuy, DisplayedMicroTxPurchasePrice, DisplayedMicroTxID, DisplayedMicroTxPurchaseName); 
	}
	else if(RightTabMode == RTAB_Armour && !class'VanillaPlusCustomization'.static.IsCharacterOwnedBy(CharacterID, SelectedTeam, SelectedClass, class'WorldInfo'.static.GetWorldInfo().GetALocalPlayerController().PlayerReplicationInfo))
	{
		LockText = class'VanillaPlusCustomization'.static.GetCharacterStoreDescription(CharacterID, bPromptToBuy, DisplayedMicroTxPurchasePrice, DisplayedMicroTxID, DisplayedMicroTxPurchaseName); 
	}
	else if(RightTabMode == RTAB_Pattern && !class'VanillaPlusCustomization'.static.IsTabardOwnedBy(TeamCustomizationChoices[SelectedTeam].ClassCustomizationChoices[SelectedClass].Tabard, TeamCustomizationChoices[SelectedTeam].ClassCustomizationChoices[SelectedClass].Character, class'WorldInfo'.static.GetWorldInfo().GetALocalPlayerController().PlayerReplicationInfo, SelectedClass))
	{
		LockText = class'VanillaPlusCustomization'.static.GetTabardStoreDescription(TeamCustomizationChoices[SelectedTeam].ClassCustomizationChoices[SelectedClass].Tabard, CharacterID, bPromptToBuy, DisplayedMicroTxPurchasePrice, DisplayedMicroTxID, DisplayedMicroTxPurchaseName);
	}
	else if(RightTabMode == RTAB_Shield && !class'VanillaPlusCustomization'.static.IsShieldPatternOwnedBy(TeamCustomizationChoices[SelectedTeam].ClassCustomizationChoices[SelectedClass].Shield, CharacterID, class'WorldInfo'.static.GetWorldInfo().GetALocalPlayerController().PlayerReplicationInfo, SelectedClass))
	{
		LockText = class'VanillaPlusCustomization'.static.GetShieldStoreDescription(TeamCustomizationChoices[SelectedTeam].ClassCustomizationChoices[SelectedClass].Shield, CharacterID, bPromptToBuy, DisplayedMicroTxPurchasePrice, DisplayedMicroTxID, DisplayedMicroTxPurchaseName);
	}
	else if(RightTabMode == RTAB_Helmet && !class'VanillaPlusCustomization'.static.IsHelmetOwnedBy(TeamCustomizationChoices[SelectedTeam].ClassCustomizationChoices[SelectedClass].Helmet, CharacterID, class'WorldInfo'.static.GetWorldInfo().GetALocalPlayerController().PlayerReplicationInfo, SelectedClass))
	{
		LockText = class'VanillaPlusCustomization'.static.GetHelmetStoreDescription(TeamCustomizationChoices[SelectedTeam].ClassCustomizationChoices[SelectedClass].Helmet, CharacterID, bPromptToBuy, DisplayedMicroTxPurchasePrice, DisplayedMicroTxID, DisplayedMicroTxPurchaseName); 
	}
		    ...
		    !AOCPlayerController(GetPC()).OwnsWeaponDrop(TeamWeaponChoices[SelectedTeam].Classes[SelectedClass].Weapons[SelectedWeapon]))
	{
		...
		if(DropInfo.GearData.AppIDNoDLC > 0)
		{
			...
			{
				bLocked = !class'VanillaPlusCustomization'.static.IsGearOwnedBy(DropInfo.GearData, class'WorldInfo'.static.GetWorldInfo().GetALocalPlayerController().PlayerReplicationInfo, SelectedClass);
				if(bLocked)
					LockText = class'VanillaPlusCustomization'.static.GetGearStoreDescription(DropInfo.GearData, bPromptToBuy, DisplayedMicroTxPurchasePrice, DisplayedMicroTxID, DisplayedMicroTxPurchaseName);
			}
			else
				bLocked = false;
		}
